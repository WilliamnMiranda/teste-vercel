...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    doadorOrgaos: "Não informado", prenhez: "Não",
    restricaoReligiosa: "", deficienciaFisica: "", imunossuprimido: "Não",
  ...
  const handleSelection = (campo, valorBooleano) => {
    ...
    if (campo === "prenhez") setFormData(prev => ({ ...prev, prenhez: valorBooleano ? "Sim" : "Não" }));
  ...
  const handleSubmit = async (e) => {
    ...
    if ((toggles.alergias && !formData.alergias.trim()) || (toggles.medicamentos && !formData.medicamentos.trim()) || (toggles.condicoes && !formData.condicoes.trim()) || (toggles.religiao && !formData.restricaoReligiosa.trim()) || (toggles.deficiencia && !formData.deficienciaFisica.trim())) { alert("⚠️ Você marcou 'SIM' em uma opção mas não descreveu os detalhes."); return; }
    if (!formData.nome || !formData.sobrenome || !formData.nascimento || !formData.genero || !formData.altura || !formData.peso || !formData.contatoNome || !formData.contatoTelefone || !formData.contatoRelacao) { alert("⚠️ Preencha todos os campos obrigatórios."); return; }
    
    setLoading(true);
    try {
      const docRef = await addDoc(collection(db, "pacientes"), { ...formData, fotoUrl: imagemBase64 || "", dataCriacao: new Date() });
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="text-center">
            ...
            <h1 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">
              {idGerado ? "Prontuário Gerado!" : "Prontuário APH"}
            </h1>
            <p className="text-slate-500 max-w-md mx-auto">Campos marcados com <span className="text-orange-600 font-bold bg-orange-100 px-1 rounded text-xs">OBRIGATÓRIO</span> precisam ser respondidos.</p>
          </div>

          {idGerado ? (
             <div className="bg-white rounded-3xl shadow-xl border border-blue-100 p-8 text-center animate-fade-in">
               <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="3"><path d="M20 6 9 17l-5-5"/></svg></div>
               <h2 className="text-2xl font-bold text-slate-800 mb-2">Cadastro Concluído!</h2>
               <p className="text-slate-500 mb-8">Imprima o cartão ou salve o QR Code.</p>
               <div className="flex justify-center mb-8 p-4 bg-white border-4 border-slate-100 rounded-2xl inline-block"><QRCodeCanvas id="qr-code-canvas" value={urlFinal} size={200} level={"H"} includeMargin={true} /></div>
               <div className="flex flex-col gap-3">
                 <button onClick={baixarQRCode} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700">Baixar Imagem QR Code</button>
                 <button onClick={() => window.location.reload()} className="w-full py-4 bg-white text-slate-700 border-2 border-slate-200 rounded-xl font-bold hover:bg-slate-50">Cadastrar Outro</button>
               </div>
             </div>
          ) : (
            <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                        <div className="flex justify-between items-center mb-3">
                             <span className="font-bold text-slate-700 text-sm">Doador de Órgãos?</span>
                             <StatusBadge respondido={toggles.doador} />
                        </div>
                        <div className="flex gap-4">
                            <button type="button" onClick={() => handleSelection("doador", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.doador === false ? 'bg-slate-600 text-white border-slate-700' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>NÃO</button>
                            <button type="button" onClick={() => handleSelection("doador", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.doador === true ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-emerald-50'}`}>SIM</button>
                        </div>
                    </div>
                    ...
                    {(formData.genero === 'Feminino' || formData.genero === '') && (
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <div className="flex justify-between items-center mb-3">
                                <span className="font-bold text-slate-700 text-sm">Suspeita de Gravidez?</span>
                                <StatusBadge respondido={toggles.prenhez} />
                            </div>
                            <div className="flex gap-4">
                    ...
                    <div className={`p-4 rounded-xl border transition-all ${toggles.deficiencia ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-100'}`}>
                        ...
                        <div className="flex gap-4 mb-3">
                            <button type="button" onClick={() => handleSelection("deficiencia", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.deficiencia === false ? 'bg-slate-600 text-white border-slate-700' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>NÃO</button>
                            <button type="button" onClick={() => handleSelection("deficiencia", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.deficiencia === true ? 'bg-blue-500 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-blue-50'}`}>SIM</button>
                        </div>
                        {toggles.deficiencia && <input name="deficienciaFisica" placeholder="Qual? (Ex: Amputação membro inferior esq.)" value={formData.deficienciaFisica} onChange={handleChange} className="w-full px-4 py-2 bg-white rounded-lg border border-blue-300 focus:ring-2 focus:ring-blue-500 outline-none" required />}
                    </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 md:p-8">
                ...
                <div className="space-y-6">
                  <div className={`p-4 rounded-2xl border-2 transition-colors ${toggles.alergias ? 'border-red-400 bg-red-50' : 'border-slate-100 bg-slate-50'}`}>
                    <div className="flex justify-between items-center mb-3"><label className="font-bold text-slate-700 text-sm">Alergias (Medicamentos/Alimentos)?</label><StatusBadge respondido={toggles.alergias} /></div>
                    <div className="flex gap-4 mb-3">
                        <button type="button" onClick={() => handleSelection("alergias", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.alergias === false ? 'bg-slate-600 text-white' : 'bg-white text-slate-500'}`}>NÃO</button>
                        <button type="button" onClick={() => handleSelection("alergias", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.alergias === true ? 'bg-red-500 text-white' : 'bg-white text-slate-500'}`}>SIM</button>
                    </div>
                    {toggles.alergias && <textarea name="alergias" rows="2" placeholder="Liste as alergias..." value={formData.alergias} onChange={handleChange} className="w-full px-4 py-2 rounded-xl border border-red-200 focus:ring-2 focus:ring-red-500 outline-none" required />}
                  </div>
                  <div className={`p-4 rounded-2xl border-2 transition-colors ${toggles.medicamentos ? 'border-blue-400 bg-blue-50' : 'border-slate-100 bg-slate-50'}`}>
                    <div className="flex justify-between items-center mb-3"><label className="font-bold text-slate-700 text-sm">Uso contínuo de Medicamentos?</label><StatusBadge respondido={toggles.medicamentos} /></div>
                    <div className="flex gap-4 mb-3">
                        <button type="button" onClick={() => handleSelection("medicamentos", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.medicamentos === false ? 'bg-slate-600 text-white' : 'bg-white text-slate-500'}`}>NÃO</button>
                        <button type="button" onClick={() => handleSelection("medicamentos", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.medicamentos === true ? 'bg-blue-500 text-white' : 'bg-white text-slate-500'}`}>SIM</button>
                    </div>
                    {toggles.medicamentos && <textarea name="medicamentos" rows="2" placeholder="Quais medicamentos?" value={formData.medicamentos} onChange={handleChange} className="w-full px-4 py-2 rounded-xl border border-blue-200 focus:ring-2 focus:ring-blue-500 outline-none" required />}
                  </div>
                  <div className={`p-4 rounded-2xl border-2 transition-colors ${toggles.condicoes ? 'border-orange-400 bg-orange-50' : 'border-slate-100 bg-slate-50'}`}>
                    <div className="flex justify-between items-center mb-1"><label className="font-bold text-slate-700 text-sm">Patologias / Cirurgias?</label><StatusBadge respondido={toggles.condicoes} /></div>
                    <p className="text-xs text-slate-400 mb-3">HAS, DM, Cardiopata, Hemolítico, Próteses...</p>
                    <div className="flex gap-4 mb-3">
                        <button type="button" onClick={() => handleSelection("condicoes", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.condicoes === false ? 'bg-slate-600 text-white' : 'bg-white text-slate-500'}`}>NÃO</button>
                        <button type="button" onClick={() => handleSelection("condicoes", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.condicoes === true ? 'bg-orange-500 text-white' : 'bg-white text-slate-500'}`}>SIM</button>
                    </div>
                    {toggles.condicoes && <textarea name="condicoes" rows="2" placeholder="Descreva o histórico..." value={formData.condicoes} onChange={handleChange} className="w-full px-4 py-2 rounded-xl border border-orange-200 focus:ring-2 focus:ring-orange-500 outline-none" required />}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 md:p-8 relative overflow-hidden">
                ...
                <div className={`p-4 rounded-2xl border transition-colors ${toggles.religiao ? 'bg-purple-50 border-purple-200' : 'bg-slate-50 border-slate-100'}`}>
                    <div className="flex justify-between items-center mb-3"><label className="font-bold text-slate-700 text-sm">Restrição a tratamentos?</label><StatusBadge respondido={toggles.religiao} /></div>
                    <div className="flex gap-4 mb-3">
                        <button type="button" onClick={() => handleSelection("religiao", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.religiao === false ? 'bg-slate-600 text-white' : 'bg-white text-slate-500'}`}>Não</button>
                        <button type="button" onClick={() => handleSelection("religiao", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.religiao === true ? 'bg-purple-500 text-white' : 'bg-white text-slate-500'}`}>Sim</button>
                    </div>
                    {toggles.religiao && <textarea name="restricaoReligiosa" rows="2" placeholder="Ex: Testemunha de Jeová (Não aceita sangue)" value={formData.restricaoReligiosa} onChange={handleChange} className="w-full px-4 py-2 rounded-xl border border-purple-200 focus:ring-2 focus:ring-purple-500 outline-none" required />}
                </div>
              </div>
              ...
              <div className="bg-slate-800 rounded-3xl shadow-lg border border-slate-700 p-6 md:p-8">
                ...
                <div className="flex gap-4">
                    ...
                    <button type="button" onClick={() => handleSelection("imunossuprimido", true)} className={`flex-1 py-2 rounded-xl border font-bold shadow-sm transition ${toggles.imunossuprimido === true ? 'bg-red-600 border-red-600 text-white' : 'bg-slate-800 border-slate-600 text-slate-400'}`}>SIM</button>
                </div>
