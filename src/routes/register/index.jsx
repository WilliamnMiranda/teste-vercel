...
import { useNavigate } from "react-router"; // Importante para navegar
...
const Register = () => {
  ...
  const [mostrarCarteirinha, setMostrarCarteirinha] = useState(false); // Estado do Modal
  ...
  // === FUNÇÕES AUXILIARES PARA A CARTEIRINHA ===
  ...
  return (
    <div className="flex-row w-full mt-12 font-sans text-slate-600">
      ...
      {/* Estilo para Impressão (caso abra o modal aqui) */}
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          {idGerado ? (
             ...
             <div className="bg-white rounded-3xl shadow-xl border border-blue-100 p-8 text-center animate-fade-in">
               ...
               <p className="text-slate-500 mb-8">O que você deseja fazer agora?</p>
               ...
               <div className="grid gap-3 max-w-sm mx-auto">
                 ...
                 <button onClick={() => navigate(`/data/${idGerado}`)} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition flex justify-center items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
               ...
               {/* ... SEU FORMULÁRIO (MANTENHA O MESMO CÓDIGO AQUI, NÃO MUDEI NADA PRA BAIXO) ... */}
               {/* MANTENHA TODO O RESTO DO CÓDIGO DO FORMULÁRIO IGUAL AO ANTERIOR */}
               {/* PARA ECONOMIZAR ESPAÇO, VOU RESUMIR, MAS VOCÊ COPIA O FORMULARIO COMPLETO DO EXEMPLO ANTERIOR */}
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 md:p-8">
                ...
                <div className="space-y-6">
                    ...
                    {/* Prenhez */}
                    {(formData.genero === 'Feminino' || formData.genero === '') && (<div className="p-4 bg-slate-50 rounded-xl border border-slate-100"><div className="flex justify-between items-center mb-3"><span className="font-bold text-slate-700 text-sm">Suspeita de Gravidez?</span><StatusBadge respondido={toggles.prenhez} /></div><div className="flex gap-4"><button type="button" onClick={() => handleSelection("prenhez", false)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.prenhez === false ? 'bg-slate-600 text-white border-slate-700' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>NÃO</button><button type="button" onClick={() => handleSelection("prenhez", true)} className={`flex-1 py-2 rounded-lg border font-bold shadow-sm transition ${toggles.prenhez === true ? 'bg-pink-500 text-white border-pink-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-pink-50'}`}>SIM</button></div></div>)}
                    {/* Deficiência */}
